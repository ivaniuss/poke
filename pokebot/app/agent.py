...
# ── Meta Guide (expert item strategy knowledge) ──────────────────────────────
_META_GUIDE_PATH = Path(__file__).parent.parent / "knowledge_base" / "meta_guide.md"

def _load_meta_guide() -> str:
    if _META_GUIDE_PATH.exists():
        return _META_GUIDE_PATH.read_text(encoding="utf-8")
    logger.warning("Meta guide not found at %s", _META_GUIDE_PATH)
    return ""

META_GUIDE = _load_meta_guide()
...
def analyst(state: AgentState) -> dict:
    from app.tools import get_item_details, ITEMS
    import re
    
    raw_data = state.get("tool_result", "")
    pokemon_names = state.get("pokemon_names", [])
    pokemon_name = pokemon_names[0] if pokemon_names else "Unknown"
    ...
    # Fetch full specs + descriptions for all mentioned items
    specs = []
    ...
    # Build Pokemon context for better reasoning
    pokemon_context = ""
    if pdata:
        types_str = ", ".join(pdata.get("types", []))
        pokemon_context = (
            f"Pokemon: {pokemon_name} | Types: {types_str} | "
    ...
    if "Data quality: HIGH" in raw_data:
        instruction = (
            ...
            "CONFIRM and EXPLAIN the top items from that data using the ITEM DESCRIPTIONS to justify why each item is good. "
        ...
        instruction = (
            "No high-ELO data available. Use the META GUIDE to recommend the best 3 items for this Pokemon's role and synergies. "
            "Consider the Pokemon's types, range, maxPP and skill to pick the optimal build. "
            "Explain WHY each item is good using the ITEM DESCRIPTIONS. "
            "This is an expert recommendation based on game mechanics, not a guess."
        )
