...
# ── Item role scoring ─────────────────────────────────────────────────────────
def _score_item_for_role(item: dict, p: dict, role: str) -> float:
    s = item["stats"]
    name = item["name"]
    score = 0.0
    ...
    # 1. Base Stat Scoring
    if role == "carry_ap":
        score = s["ap"] * 3 + s["pp"] * 1.5 + s["speed"] * 0.5
    elif role == "carry_atk":
        score = s["atk"] * 3 + s["critChance"] * 1.5 + s["critPower"] * 0.5 + s["speed"] * 1.0
    elif role == "tank":
        score = s["hp"] * 1.5 + s["def"] * 3 + s["speDef"] * 2.5 + s["shield"] * 1.0
    elif role == "support":
        score = s["pp"] * 2 + s["shield"] * 2 + s["luck"] * 1.5 + s["speDef"] * 1.0
    else:
        score = (s["ap"] + s["atk"] * 2 + s["hp"] + s["def"] + s["speDef"]) * 1.0

    # 2. Strategic Multipliers (Matching Meta Guide)

    # EVIOLITE: Huge boost for non-evolved units
    if name == "EVIOLITE" and p.get("evolution"):
        score += 300  # High flat boost to make it competitive with BiS

    # SKILL LOGIC: Low PP units want multipliers, high PP units want regen
    max_pp = p.get("maxPP", 0)
    if max_pp > 0:
        if max_pp <= 60:
            # Low PP: Boost raw damage and crit-ability
            if name in ["REAPER_CLOTH", "CHOICE_SPECS", "RAZOR_CLAW"]:
                score *= 1.8
            # Low PP: Penalize excessive PP regen
            if name in ["AQUA_EGG", "MYSTIC_WATER"]:
                score *= 0.5
        elif max_pp >= 120:
            # High PP: Boost PP regen items significantly
            if name in ["AQUA_EGG", "SOUL_DEW", "DEEP_SEA_TOOTH", "MYSTIC_WATER"]:
                score *= 2.5

    # RANGE LOGIC: Melee carries (range 1) need safety
    p_range = p.get("range", 1)
    if p_range == 1 and role.startswith("carry"):
        if name in ["KINGS_ROCK", "PROTECTIVE_PADS", "POKE_DOLL"]:
            score += 150
    elif p_range >= 3 and role == "carry_ap":
        # Backline AP carries love raw glass-cannon items
        if name in ["CHOICE_SPECS", "REAPER_CLOTH"]:
            score *= 1.3

    # SYNERGY-SPECIFIC (Simplified)
    p_types = set(p.get("types", []))
    if "GHOST" in p_types and name == "REAPER_CLOTH":
        score *= 1.2
    if "PSYCHIC" in p_types and name in ["CHOICE_SPECS", "SOUL_DEW"]:
        score *= 1.2
    if "WATER" in p_types and name == "AQUA_EGG":
        score *= 1.2
...
def recommend_items(pokemon_name: str, role: str = "auto") -> str:
    ...
    if has_bot_data:
        ...
        scored = sorted(ITEMS.items(), key=lambda kv: _score_item_for_role(kv[1], p, effective_role), reverse=True)
